...
class IterativeAgentWithToolAndRollback(BaseAgent):
    ...
    async def run_internal(self, ctx: AgentContext) -> AgentContext:
        ...
        if ctx.get("message_history") is None:
            # When using native tool calls, skip MCP XML definitions in system prompt
            # (tools are passed via API parameter instead)
            use_tool_calls = getattr(self.llm_client, "use_tool_calls", False)
            mcp_defs = None if use_tool_calls else self.mcp_server_definitions
            input_processor_output = await self.input_processor.run(
                AgentContext(**ctx, mcp_server_definitions=mcp_defs)
            )
        ...
        while max_turns == -1 or turn_count < max_turns:
            ...
            # Support both single message (dict) and multiple messages (list)
            # Native tool_calls mode returns a list of role:"tool" messages
            if isinstance(user_msg, list):
                message_history.extend(user_msg)
            else:
